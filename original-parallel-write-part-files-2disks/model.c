...
// {
      ...
      {
      // double dd1 = wtime();
      DumpSliceFile_Parallel_Nofor(sx,sy,sz,pc,sPtr,pn,&diskid);
      // tdt+=wtime()-dd1;
      }
// }
...
#define GIGA 1.0e-9
  ...
  printf("original-parallel-write-part-files-2disks,%s,%d,%d,%d,%d,%.2f,%.2f,%.2f,%f,%f,%lu,%lu,%lf,%lf,%.0lf\n", 
  ...
  for (int p = 1; p <= sPtr->itCnt - 1; p++) {
    char partFileName[256];
    sprintf(partFileName, "%s%d.bin", "TTI_part_", p);
    FILE* partFile = fopen(partFileName, "rb");

    printf("copying part: %s\n", partFileName);

    size_t n, m;
    unsigned char buff[8192];
    do {
        n = fread(buff, 1, sizeof buff, partFile);
        if (n)
          m = fwrite(buff, 1, n, sPtr->fpBinary);
        else
          m = 0;
    } while ((n > 0) && (n == m));
    if (m)
